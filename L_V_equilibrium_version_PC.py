...
##
    ...
    while True:
        ...
        for k, mol in enumerate(['CO2', 'NH3', 'CH4', 'Ar', 'Kr', 'Xe']):
            ...
            if mol == 'CO2'  :                      # For CO2
                ...
                if np.abs(tolerances[k]) > 2e-3 * M_bulk[k]:  # Check if tolerance is still unmet
                    print(f"Checking {mol}: Tolerance = {tolerances[k]}, Condition = {tolerances[k] > 0.01 * M_bulk[k]}")
                    all_met = False  # Set flag to false if any tolerance is not met
                    # Update epsilon based on the tolerance condition
                    if np.abs(tolerances[k]) < 0.01 * M_bulk[k]:
                        epsilon = epsilon_low[k] 
                    else:
                        epsilon = epsilon_high[k]
                ...
                if np.abs(tolerances[k]) > 5e-3 * M_bulk[k]:
                ...
                if np.abs(tolerances[k]) > 1e-3 * M_bulk[k]:
                    print(f"Checking {mol}: Tolerance = {tolerances[k]}, Condition = {tolerances[k] > 0.05 * M_bulk[k]}")
                    all_met = False
                    epsilon = epsilon_low[k] if np.abs(tolerances[k]) < 0.01 * M_bulk[k] else epsilon_high[k]
                    if M_bulk[2]> 4e16:
                        epsilon = epsilon/20
                ...
                if np.abs(tolerances[k]) > 3e-3 * M_bulk[k]:
                    print(f"Checking {mol}: Tolerance = {tolerances[k]}, Condition = {tolerances[k] > 0.1 * M_bulk[k]}")
                    all_met = False
                    epsilon = epsilon_low[k] if np.abs(tolerances[k]) < 0.01 * M_bulk[k] else epsilon_high[k]
                    if M_bulk[k]<1e15:
   ...
   # User input : total molality of CO2 and NH3 :
    ...
    sol = least_squares(Chemical_eq_fun, z0, method='lm', ftol=1e-8, xtol=1e-8)
    ...
    z0 = [PsatH2O, PRES[2]*1e5, PRES[1]*1e5]
    ...
    composition_liquidphase['pH'][i] = -np.log10(m_Hp)

    sol = least_squares(L_V_eq_fun, z0, method='lm', ftol=1e-8, xtol=1e-8)
